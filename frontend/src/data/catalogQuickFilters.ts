/**
 * Базовые кнопки моделей в каталоге.
 *
 * Они одновременно служат безопасным стартовым набором для новой категории
 * и резервом для старых категорий, в которых список ещё не был сохранён в БД.
 * После сохранения в админке приоритет всегда у списка из API.
 */
...
export const DEFAULT_CATALOG_QUICK_FILTERS: Record<string, CatalogQuickFilter[]> = {
  smartphones: [
...
export function getDefaultCatalogQuickFilters(slug?: string | null): CatalogQuickFilter[] {
  return (DEFAULT_CATALOG_QUICK_FILTERS[slug || ''] ?? []).map(filter => ({ ...filter }))
}
